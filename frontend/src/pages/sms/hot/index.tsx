...
import { homeHotApi, type HomeRecommend } from '@/api/homeRecommend'
...
export default function HomeHotPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const res = await homeHotApi.list({ productName, recommendStatus, pageNum, pageSize })
  ...
  const handleStatusChange = async (record: HomeRecommend, checked: boolean) => {
    await homeHotApi.updateRecommendStatus({ ids: [record.id], recommendStatus: checked ? 1 : 0 })
    message.success('推荐状态修改成功')
    fetchData()
  }

  const handleDelete = async (id: number) => {
    await homeHotApi.delete([id])
    message.success('删除成功')
    fetchData()
  }

  const handleBatchDelete = async () => {
    await homeHotApi.delete(selectedIds)
  ...
  const handleBatchStatus = async (recommendStatusValue: number) => {
    await homeHotApi.updateRecommendStatus({ ids: selectedIds, recommendStatus: recommendStatusValue })
