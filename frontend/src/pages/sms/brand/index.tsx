...
import { homeBrandApi, type HomeRecommend } from '@/api/homeRecommend'
...
export default function HomeBrandPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const res = await homeBrandApi.list({ brandName, recommendStatus, pageNum, pageSize })
  ...
  const handleStatusChange = async (record: HomeRecommend, checked: boolean) => {
    await homeBrandApi.updateRecommendStatus({ ids: [record.id], recommendStatus: checked ? 1 : 0 })
    message.success('推荐状态修改成功')
    fetchData()
  }

  const handleDelete = async (id: number) => {
    await homeBrandApi.delete([id])
    message.success('删除成功')
    fetchData()
  }

  const handleBatchDelete = async () => {
    await homeBrandApi.delete(selectedIds)
  ...
  const handleBatchStatus = async (recommendStatusValue: number) => {
    await homeBrandApi.updateRecommendStatus({ ids: selectedIds, recommendStatus: recommendStatusValue })
