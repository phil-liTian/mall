...
import { homeSubjectApi, type HomeRecommend } from '@/api/homeRecommend'
...
export default function HomeSubjectPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const res = await homeSubjectApi.list({ subjectName, recommendStatus, pageNum, pageSize })
  ...
  const handleStatusChange = async (record: HomeRecommend, checked: boolean) => {
    await homeSubjectApi.updateRecommendStatus({ ids: [record.id], recommendStatus: checked ? 1 : 0 })
    message.success('推荐状态修改成功')
    fetchData()
  }

  const handleDelete = async (id: number) => {
    await homeSubjectApi.delete([id])
    message.success('删除成功')
    fetchData()
  }

  const handleBatchDelete = async () => {
    await homeSubjectApi.delete(selectedIds)
  ...
  const handleBatchStatus = async (recommendStatusValue: number) => {
    await homeSubjectApi.updateRecommendStatus({ ids: selectedIds, recommendStatus: recommendStatusValue })
  ...
  const columns: ColumnsType<HomeRecommend> = [
    { title: '专题名称', dataIndex: 'subjectName' },
    {
      title: '推荐状态',
